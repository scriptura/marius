...
pub enum PageComposeParseError {
    ...
    ///
    /// Portée Phase 4.4 : cette variante couvre aussi, temporairement, tout
    /// mot-clé de bloc encore non représentable par `PageSourceToken` à ce
    /// stade du classifieur (`static`, `extends`, et tout mot-clé hors
    /// grammaire runtime) — Document 1 §3 autorise explicitement l'échec de
    /// ces fichiers avant la clôture du classifieur (Phase 4.7). `block` et
    /// `endblock` en sont sortis en Phase 4.4 (branche `Block` dédiée) ;
    /// `static` et `extends` migreront à leur tour (`Static` en 4.5,
    /// `extends` en 4.6, `Unsupported` en 4.7) sans que cette variante
    /// d'erreur ne soit retirée : elle reste le domaine des erreurs de
    /// grammaire structurelle des mots-clés déjà reconnus (`if`/`endif`/
    /// `block`/`endblock`) — par exemple un `{% if %}` sans point dans
...
pub struct StaticPartialRef<'src> {
    ...
    /// `relative_path_for_include_str` sans adaptation.
...
//   - N'implémente pas encore `extends` (4.6), `{% static %}` (4.5), ni le
//     catch-all `Unsupported` (4.7) : tout mot-clé de bloc autre que
//     `if`/`endif`/`block`/`endblock` — y compris `extends`, `static`,
//     `include` (absent de la grammaire Mode Page par construction du type
//     `PageSourceToken::Runtime`, cf. Phase 4.1) — échoue avec
//     `PageComposeParseError::InvalidBlockSequence` à ce stade.
//     Explicitement hors scope de ce diff (roadmap §4.3) : ce comportement
//     n'anticipe aucune décision des phases suivantes, il migre mot-clé par
//     mot-clé lorsque chacune sera implémentée.
//   - `{% block %}` / `{% endblock %}` : voir section Phase 4.4 ci-dessous,
//     qui étend `parse_page_block` (seule fonction modifiée) sans toucher à
//     ce dispatch de tête.
...
// `{% static %}` (4.5), `extends` (4.6) et le catch-all `Unsupported` (4.7)
// restent hors périmètre : ils continuent d'échouer via
// `PageComposeParseError::InvalidBlockSequence`, comportement inchangé par
// ce diff.
...
/// `parse_page_tokens`.
///
/// Portée Phase 4.4 : reconnaît `if`/`endif` (Phase 4.3, logique inchangée)
/// et `block`/`endblock` (introduits ici). Tout autre mot-clé (`extends`,
/// `static`, `include`, ou inconnu) retourne toujours
/// `InvalidBlockSequence` — comportement temporaire, qui migrera mot-clé
/// par mot-clé dans les phases suivantes (4.5 `static`, 4.6 `extends`,
...
/// `PageSourceToken::Block`, jamais sous `Runtime`. `if`/`endif`, eux,
/// restent enveloppés sous `Runtime`, exactement comme en Phase 4.3. Cette
/// fonction devient donc le site d'enveloppe unique pour tout mot-clé de
/// bloc reconnu : `parse_page_tokens` (dispatch de tête) n'a plus à choisir
/// l'enveloppe lui-même, il propage tel quel ce qui est retourné ici — un
/// seul endroit décide de la variante `PageSourceToken`, cohérent avec la
/// règle « le Parser ne décide qu'une fois de la forme d'un token ».
///
...
where
    I: Iterator<Item = RawSpan<'src>>,
{
    let keyword = expect_ident_page(iter, "keyword (if | endif | block | endblock)")?;
