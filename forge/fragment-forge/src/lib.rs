...
pub enum PageComposeParseError {
    ...
    ///
    /// Portée Phase 4.3 : cette variante couvre aussi, temporairement, tout
    /// mot-clé de bloc encore non représentable par `PageSourceToken` à ce
    /// stade du classifieur (`block`/`endblock`, `static`, `extends`, et tout
    /// mot-clé hors grammaire runtime) — Document 1 §3 autorise explicitement
    /// l'échec de ces fichiers avant la clôture du classifieur (Phase 4.7).
    /// Chacun de ces cas migrera vers une branche dédiée du classifieur
    /// (`Block` en 4.4, `Static` en 4.5, `extends` en 4.6, `Unsupported` en
    /// 4.7) sans que cette variante d'erreur ne soit retirée : elle reste le
    /// domaine des erreurs de grammaire structurelle du sous-ensemble `if` /
    /// `endif`.
    InvalidBlockSequence,
}
...
//   - N'implémente pas encore `extends` (4.6), `{% block %}` (4.4),
//     `{% static %}` (4.5), ni le catch-all `Unsupported` (4.7) : tout
//     mot-clé de bloc autre que `if`/`endif` — y compris `extends`,
//     `block`/`endblock`, `static`, `include` (absent de la grammaire Mode
//     Page par construction du type `PageSourceToken::Runtime`, cf. Phase
//     4.1) — échoue avec `PageComposeParseError::InvalidBlockSequence` à ce
//     stade. Explicitement hors scope de ce diff (roadmap §4.3) : ce
//     comportement n'anticipe aucune décision des phases suivantes, il migre
//     mot-clé par mot-clé lorsque chacune sera implémentée.
...
/// information devient calculable. Ne reconnaît aucun mot-clé de composition
/// (voir note de portée ci-dessus).
///
/// ─── Invariants mémoire ─────────────────────────────────────────────────────
///
...
pub fn parse_page_tokens<'src>(
    ...
    while let Some(span) = iter.next() {
        let token = match span.kind {
            ...
            // `{% keyword … %}` → IfBool | EndIf (seuls mots-clés reconnus à
            // ce stade), enveloppé Runtime.
            SpanKind::BlockOpen => PageSourceToken::Runtime(parse_page_block(&mut iter)?),
...
/// Consomme `Ident(keyword) … BlockClose` et produit `IfBool`/`EndIf`.
/// Précondition : `BlockOpen` vient d'être consommé par `parse_page_tokens`.
///
/// Portée Phase 4.3 : seuls `if` et `endif` sont reconnus. Tout autre
/// mot-clé (`extends`, `block`, `endblock`, `static`, `include`, ou
/// inconnu) retourne `InvalidBlockSequence` — comportement temporaire,
/// documenté au niveau de la variante d'erreur elle-même.
fn parse_page_block<'src, I>(iter: &mut I) -> Result<FlatPageToken<'src>, PageComposeParseError>
where
    I: Iterator<Item = RawSpan<'src>>,
{
    let keyword = expect_ident_page(iter, "keyword (if | endif)")?;
