...
//! Pipeline Mode Page (héritage `extends`/`block`, imports `{% import %}`) :
//! `model` → `token` → `parser` → `blocks` → `importer` → `linker` →
//! `lowering`. Point de jonction unique avec le pipeline Fragment :
//! `lowering::lower` produit un `Vec<FlatPageToken>` qui réintègre
//! `fragment::resolver` / `fragment::codegen`.
...
pub use importer::collect_top_level_imports;
