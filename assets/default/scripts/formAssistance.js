...
const formattedUsername = (() => {
	const username = document.querySelector(
		"#input-username.assistance-username",
	);
...
(function createSlug() {
	const inputName = document.querySelector(
		"#input-name.assistance-slug, #input-username.assistance-slug",
	);
	...
	if (inputName && inputSlug)
		inputName.addEventListener("input", () => slugFormat(inputName, inputSlug));
})();
...
document
	.querySelectorAll(".character-counter")
	.forEach((input) => characterCounter(input));
...
function characterCounterDecremental(input) {
	...
	const decrement = () => {
		const text = input.value;
		const count = maxLength - text.length;
		output.textContent = count;
	};
...
document
	.querySelectorAll(".character-counter-decremental")
	.forEach((input) => characterCounterDecremental(input));
...
document
	...
	.forEach((textarea) => textareaAutosize(textarea));
...
document
	.querySelectorAll(".assistance-delete")
	.forEach((button) => deleteArticle(button));
